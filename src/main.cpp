...
#if defined(NDEBUG)
# error "OroCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdb74b58ea317cf666816c4fb43085f979025e1fd45354fbeb6aadddf6de08b40");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OroCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "OroCoin Signed Message:\n";
...
{
    // OroCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // OroCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // OroCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1484631908, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "NYT 16 Jan 2017 A Sense of Uncertainty Grips the World";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e116e39050ed071e762ab693516e0601e7f874dd6baf97847cfd97325b191675938f56e30ecf5db1e5ac1c68d776d762b70177c93d241e280af61c4033a5b8b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1484631908;
        ...
        assert(block.hashMerkleRoot == uint256("0x32a6f467e43d1bcdc3b032f6087bdfbe084953346d745392fc69a2cd59f02bc6"));
...
unsigned char pchMessageStart[4] = { 0x32, 0x10, 0x35, 0xa5 }; // OroCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OroCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OroCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OroCoinMiner(CWallet *pwallet)
{
    printf("OroCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("orocoin-miner");
    ...
    try { loop {
        ...
        printf("Running OroCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OroCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OroCoinMiner, pwallet));
}
